//! Ported from the kernel's src/vga.rs. The one real difference: `buffer`
//! is set at runtime from the address `map_memory` mapped the VGA buffer
//! to (see main.rs), not a compile-time constant -- this task doesn't have
//! the kernel's identity-mapped alias, only whatever it asked the kernel
//! to map for it.
...
impl Writer {
    pub fn new(buffer: *mut u16) -> Self {
        Writer {
            col: 0,
            row: 0,
            fg: Color::LightGray,
